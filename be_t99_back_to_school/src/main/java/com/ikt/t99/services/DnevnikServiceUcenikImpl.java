...
@Service
public class DnevnikServiceUcenikImpl implements DnevnikServiceUcenik {

	private final Logger logger = LoggerFactory.getLogger(DnevnikServiceUcenikImpl.class);
	...
	public ResponseEntity<?> listByParam(HttpServletRequest request, String datum, String datum_do,
		...
		// Instanciram ucenikEntity.
		UcenikEntity ucenikEntity = new UcenikEntity();
		try {
			ucenikEntity = ucenikRepository.findByKorisnickoIme(request.getUserPrincipal().getName());
			logger.info("Ucenik instanciran.");
		} catch (Exception e) {
			return ResponseEntity
					.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("Došlo je do greške prilikom instanciranja učenika: " + e.getMessage());
		}
		
		// Validiram prosledjene parametre i dinamicki sastavljam hql string.
		StringBuilder hqlString = new StringBuilder("SELECT d FROM DnevnikEntity d WHERE d.ucenik.id = :ucenik_id");
        ...
        TypedQuery<DnevnikEntity> dnevnikQuery = entityManager.createQuery(hqlString.toString(),
        		DnevnikEntity.class);
        dnevnikQuery.setParameter("ucenik_id", ucenikEntity.getUcenik_id());
        if (datum != null) {
