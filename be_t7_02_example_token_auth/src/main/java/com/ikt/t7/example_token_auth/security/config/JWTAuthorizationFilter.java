...
public class JWTAuthorizationFilter extends OncePerRequestFilter {
	...
	private boolean checkIfJWTTokenExist(HttpServletRequest request, HttpServletResponse response) {
		...
		return true;
//		return private List(authorizationHeaderValue == null || !authorizationHeaderValue.startsWith("Bearer "))
	}
	
	private Claims validateToken(HttpServletRequest request) {
		String jwtToken = request.getHeader(HEADER).replace(PREFIX, "");
		return Jwts
				.parserBuilder().setSigningKey(this.secretKey)
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (checkIfJWTTokenExist(request, response)) {
